...
class Trivia {
  ...
  async get_data(dif, cat) {
    ...
    if (dif && !cat) {
      ...
      if (
        ...
        return this.message.reply(
          `〔 ${this.client.emotes.error} 〕**Ingresa un dificultad válida.** \`easy\`, \`medium\`, \`hard\`, \`any\`.`
        );
      await fetch(
    ...
    if (dif && cat) {
      ...
      if (!this.question_id)
        //SHEEEEEEESH VA A MANDAR TAREA, PERAME 2 MIN OKI
        return this.message.reply(
          `〔 ${this.client.emotes.error} 〕**Ocurrió un error inesperado.**`
        );
  ...
  async show_question() {
    if (atob(this.question.results[0].type) == "multiple") {
      ...
      if (this.correct_answer == 1) {
        this.answer_array = [
          "**`A.`** " +
            (await Transale(atob(this.question.results[0].correct_answer))),
          "**`B.`** " +
            (await Transale(
      ...
      const TransLateReadyUwU = await gtranslate(
        atob(this.question.results[0].question),
        { from: "en", to: "es" }
      );
    ...
    if (atob(this.question.results[0].type) == "boolean") {
      ...
      const TransLateReady = await gtranslate(
        atob(this.question.results[0].question),
        { from: "en", to: "es" }
      );
      this.question_embed = new Discord.MessageEmbed()
    ...
    this.question_message = await this.message.reply(this.question_embed);
    let step = -1;
    ...
    this.question_message
      ...
      .then(async (collected) => {
        ...
        if (this.input_answer == this.correct_answer) {
          ...
          const TransLateReady2 = await gtranslate(
            atob(this.question.results[0].question),
            { from: "en", to: "es" }
          );
          this.question_embed = new Discord.MessageEmbed()
          ...
          this.question_message
            .edit(`<@!${this.player}>, has elegido correctamente 😄.`)
            .catch(() => {});
          this.end_game();
        } else {
          this.answer_array[this.input_answer - 1] =
          ...
          const TransLateReady3 = await gtranslate(
            atob(this.question.results[0].question),
            { from: "en", to: "es" }
          );
          this.question_embed = new Discord.MessageEmbed()
          ...
          this.question_message
            .edit(
              `<@!${
                this.player
              }>, no has elegido correctamente. La respuesta correcta fue ${
      ...
      .catch(() => {
        this.question_message
          .edit(
            `<@!${
              this.player
            }>, tardaste demasiado en responder! Se agotó el tiempo de espera del juego. La respuesta fue ${
              this.reactions[this.correct_answer - 1]
            }.`
          )
          .catch(() => {});
  ...
  async end_game() {
    if (this.message.guild.me.permissions.has("MANAGE_EMOJIS")) {
